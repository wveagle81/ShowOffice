...
sub members_by_lastname {
   ...
   open (HTML_REPORT, ">temp/reportfile$$.html") or die "Unable to open temp file: $!";
   ...
   $IE->{visible} = 1;
   $IE->Navigate("file:///C:/Perl%20Programs/ShowOffice/temp/reportfile$$.html");
}
...
sub national_back_number {
   ...
   foreach $backrider (sort keys %blist) {
      ($backnum, $ridernum) = split(/~/, $backrider);
      ...
      $riderinfo = "${ridername} / ${ridernum}";
      $ownerinfo = "${ownername} / ${ownernum}";
...
print "Doing class winner\n";
      ...
      open (CLASSENTRIES, ">temp/reportfile$$.txt") or die "Unable to open temp file: $!";
      print CLASSENTRIES "";
      close CLASSENTRIES;
...
print "Class Num: $classnumber\nClass Nam: $classname\n";
      ...
      open (CLASSENTRIES, "temp/reportfile$$.txt");
...
print "Flag = $flag\n";
      if ($total_class_entries <= 0) {
         $backnumber = 0;
         $horsename = "THERE ARE NO ENTRIES";
         $ridername = "IN THIS CLASS";
...
#
   (undef,undef,$shownum1,$shownum2,$shownum3,$shownum4,$judge1,$judge2,$judge3,$judge4,$maxplace) = split(/~~/, $openshowdata);
#   $judgeinfo1 = sprintf("%5s %-20s", substr($shownum1, 4), $judge_list{$judge1}) if ($judge1);
#   $judgeinfo2 = sprintf("%5s %-20s", substr($shownum2, 4), $judge_list{$judge2}) if ($judge2);
#   $judgeinfo3 = sprintf("%5s %-20s", substr($shownum3, 4), $judge_list{$judge3}) if ($judge3);
#   $judgeinfo4 = sprintf("%5s %-20s", substr($shownum4, 4), $judge_list{$judge4}) if ($judge4);
   $judgeinfo1 = sprintf("%-20s", $judge_list{$judge1}) if ($judge1);
   $judgeinfo2 = sprintf("%-20s", $judge_list{$judge2}) if ($judge2);
   $judgeinfo3 = sprintf("%-20s", $judge_list{$judge3}) if ($judge3);
   $judgeinfo4 = sprintf("%-20s", $judge_list{$judge4}) if ($judge4);
...
#
   undef(%cle);
   @back_entries = grep(/^${backnumber}~~${horsenum}~~${ridernum}/, @entry_list);

   foreach $ii (@back_entries) {
      $total_points = 0;
      $placing_info1 = $placing_info2 = $placing_info3 = $placing_info4 = "";
      (undef, undef, undef, undef, $classnumber, $place1, $tie1, $points1, $place2, $tie2, $points2, $place3, $tie3, $points3, $place4, $tie4, $points4) = split(/~~/, $ii);
      $cle{$classnumber} = $ii;
   }
   ...
   foreach $key (sort { $a <=> $b } keys %cle) {
      @class_entries = ();
      (undef, undef, undef, undef, $classnumber, $place1, $tie1, $points1, $place2, $tie2, $points2, $place3, $tie3, $points3, $place4, $tie4, $points4) = split(/~~/, $cle{$key});
      foreach $cl (@entry_list) {
         (undef, undef, undef, undef, $cln) = split(/~~/, $cl);
         push(@class_entries, $cln) if ($cln eq $classnumber);
      }
      $total_entries = $#class_entries+1;
...
#         if ($classnumber eq "OS") { $classinfo = "OPEN GRAND / RESERVE STALLIONS"; }
      ...
      if ($place1) { $placing_info1 = sprintf("%-5s %5s %6s", substr($shownum1, 4), $place1, $points1); $national_points += $points1; }
      else         { $placing_info1 = sprintf("%-5s            0", substr($shownum1, 4)); }
      if ($place2) { $placing_info2 = sprintf("%-5s %5s %6s", substr($shownum2, 4), $place2, $points2); $national_points += $points2; }
      else         { $placing_info2 = sprintf("%-5s            0", substr($shownum2, 4)); }
      if ($place3) { $placing_info3 = sprintf("%-5s %5s %6s", substr($shownum3, 4), $place3, $points3); $national_points += $points3; }
      else         { $placing_info3 = sprintf("%-5s            0", substr($shownum3, 4)); }
      if ($place4) { $placing_info4 = sprintf("%-5s %5s %6s", substr($shownum4, 4), $place4, $points4); $national_points += $points4; }
      else         { $placing_info4 = sprintf("%-5s            0", substr($shownum4, 4)); }
...
#
   ...
   foreach $ii (@back_entries) {
      @class_entries = ();
      (undef, undef, undef, undef, $classnumber) = split(/~~/, $ii);
      $cle{$classnumber} = $ii;
   }

   foreach $key (sort { $a <=> $b } keys %cle) {
      (undef, undef, undef, undef, $classnumber) = split(/~~/, $cle{$key});
      foreach $cl (@entry_list) {
         (undef, undef, undef, undef, $cln) = split(/~~/, $cl);
         push(@class_entries, $cln) if ($cln eq $classnumber);
      }
...
#
      ...
      @cl_entry = grep (/^\d+~~[P]{0,1}\d+~~[P]{0,1}\d+~~[a-zA-Z- ]*~~$hp_class~~/, @entry_list);
...
#
      open (RESULTS, ">temp/$shownum[$index].txt") or die "Unable to open temp file: $!";
...
#
 ...
 print LOG "Class: $key  $oc{$key}[0]  $oc{$key}[1]\n";
        if ($key =~ /^G/) {
           $ckey = "O" . substr($key,1);
        } else {
           $ckey = $key;
...
#
         foreach $cle (@cl_entry) {
            ...
            $hnum = sprintf("%07d", $h);
            $rnum = sprintf("%07d", $r);
printf LOG "Entry: %-5dHorse: %-20.20sRider: %-20.20s\n", $b, $horsename, $ridername;
...
#  HEADER INFORMATION
#
      $showlocation = "San Marcos, TX";
      $judgeinfo    = "$judge[$index] : $judge_list{$judge[$index]}";

      $IE = Win32::OLE->new('InternetExplorer.Application') || die "Could not start Internet Explorer.Application\n";

      open (HTML_REPORT, ">temp/reportfile.${index}$$.html") or die "Unable to open temp file: $!";
...
#print "Class Info: $cnum  $classname\n";
      ...
      $IE->{visible} = 1;
      $IE->Navigate("file:///C:/Perl%20Programs/ShowOffice/temp/reportfile.${index}$$.html");
   }
}
...
##################################################################################
#
#       Write The National Reports To Diskette
#
...
#  NON-PRO CLASSES
#
   local $reporttype2 = "FOR $type1 CLASSES";
   &localpoints_header_html;
...
sub national_master {
   ...
   $reporttype = "IBHA SHOW RESULTS MASTER HORSE LIST";
...
#
   ...
   @open_cell    = ("B6", "C6", "D6", "E6", "F6", "G6", "H6",
                    ...
                    "B10", "C10", "D10", "E10", "F10", "G10", "H11",
                    "B11", "C11", "D11", "E11", "F11", "G10", "H11");
...
print "Adding Leadline Class To Cell ", ${lead_cell}, " For Leadline Rider\n";
                  ...
                  $sheet->Range(${lead_rider})->{Value} = "$riderinfo";
...
print "Adding Special Class To Cell ", ${special_cell}[$special_index], " For Rider #${special}\n";
                        ...
                        $sheet->Range(${special_rider}[$special])->{Value}      = "$riderinfo";
...
#   Open the existing blank worksheet
#
   $openfile = "${cwd}\\EntryFormPrepaid.xls";
   my $book = $excel->Workbooks->Open($openfile);
